...
for i in range(len(inputData)):
    inputDataMag.append(np.linalg.norm(inputData[i]))
outputDataMag = []
for i in range(len(outputData)):
    outputDataMag.append(np.linalg.norm(outputData[i]))
inputDataMag = np.array(inputDataMag)
outputDataMag = np.array(outputDataMag)
# x = np.arange(0, 3)
# expectedData = 3 * x

# inputDataMagModified = []
# for i in range(len(inputDataMag)):
#     inputDataMagModified.append(inputDataMag[i] * 1)
# plt.scatter(outputDataMag, inputDataMagModified, color='orange')
# plt.scatter(inputDataMag, outputDataMag, color='blue')
# plt.plot(x, expectedData)
# plt.xlabel("Input Data Magnitude")
# plt.ylabel("Output Data Magnitude")
...
if __name__ == '__main__':
    ...
    print(f"Input power shape: {inputDataMag.shape}")
    print(f"Output power shape: {outputDataMag.shape}")

    # Split data into training and testing sets
    X_train, X_test, y_train, y_test = train_test_split(
        inputDataMag, outputDataMag, test_size=0.2, random_state=42
    )
    ...
    # new_input_power = torch.tensor([[5.0]], dtype=torch.float32).to(device)
    # with torch.no_grad():
